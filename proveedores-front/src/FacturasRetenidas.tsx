...
import { fetchFacturasRetenidas, FacturaRetenida, mapTipoDocumento } from './api';
import { useProveedor } from './ProveedorContext';

const FacturasRetenidas: React.FC = () => {
  const { codigoProveedor } = useProveedor();
  const [facturas, setFacturas] = useState<FacturaRetenida[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);

  // Función para cargar facturas
  const cargarFacturas = async (id: string) => {
    if (!id) return;
    ...
    try {
      const data = await fetchFacturasRetenidas(id);
      setFacturas(data);
    } catch (err) {
      setError('Error al cargar las facturas retenidas');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    cargarFacturas(codigoProveedor);
  }, [codigoProveedor]);
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      <h2 style={{ marginBottom: '20px', color: '#333' }}>Facturas Retenidas</h2>
      ...
      {/* Tabla de resultados */}
      {!loading && codigoProveedor && facturas.length > 0 && (
          ...
          <table style={{ 
            ...
            <tbody>
              {facturas.map((factura, index) => (
                ...
                >
                  <td style={{ padding: '12px 16px' }}>{mapTipoDocumento(factura.doctype)}</td>
                  <td style={{ padding: '12px 16px' }}>{cleanString(factura.docnumbr)}</td>
                  <td style={{ padding: '12px 16px' }}>{formatDate(factura.docdate)}</td>
                  <td style={{ padding: '12px 16px' }}>{formatDate(factura.duedate)}</td>
                  <td style={{ padding: '12px 16px' }}>{cleanString(factura.curncyid)}</td>
                  <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                    {formatCurrency(factura.docamnt, factura.curncyid)}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {!loading && codigoProveedor && facturas.length === 0 && (
        <div style={{ 
          ...
          <span>No se encontraron facturas retenidas para este proveedor</span>
        </div>
      )}
    </div>
  );
...
export default FacturasRetenidas;