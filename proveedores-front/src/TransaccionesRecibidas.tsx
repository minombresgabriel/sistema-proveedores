...
import { fetchTransaccionesRecibidas, TransaccionRecibida, mapTipoTransaccion } from './api';
import { useProveedor } from './ProveedorContext';

const TransaccionesRecibidas: React.FC = () => {
  const { codigoProveedor } = useProveedor();
  const [transacciones, setTransacciones] = useState<TransaccionRecibida[]>([]);
  ...
  const cargarTransacciones = async (id: string) => {
    ...
    try {
      const data = await fetchTransaccionesRecibidas(id);
      setTransacciones(data);
    } catch (err) {
      setError('Error al cargar las transacciones recibidas');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      <h2 style={{ marginBottom: '20px', color: '#333' }}>Documentos por Analizar</h2>
          ...
          <table style={{ 
            ...
            <thead>
              <tr style={{ backgroundColor: '#f5f5f5' }}>
                ...
                <th style={{ padding: '12px 16px', textAlign: 'left', borderBottom: '1px solid #ddd' }}>Recibido</th>
            ...
            <tbody>
              {transacciones.map((transaccion, index) => (
                ...
                >
                  <td style={{ padding: '12px 16px' }}>{mapTipoTransaccion(transaccion.tipo)}</td>
                  <td style={{ padding: '12px 16px' }}>{cleanString(transaccion.docNumero)}</td>
                  <td style={{ padding: '12px 16px' }}>{formatDate(transaccion.fechaRecibido)}</td>
                  <td style={{ padding: '12px 16px' }}>{formatDate(transaccion.fechaVencimiento)}</td>
                  <td style={{ padding: '12px 16px' }}>{cleanString(transaccion.currency)}</td>
                  <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                    {formatCurrency(transaccion.subtotal, transaccion.currency)}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {!loading && codigoProveedor && transacciones.length === 0 && (
        <div style={{ 
          ...
          <span>No se encontraron documentos por analizar para este proveedor</span>
        </div>
      )}
    </div>
  );
